...
class Baul(projectFilesPath: String) {
    ...
    private lateinit var data: HashMap<String, BaseDataSource>
    ...
    fun loadFromCache() {
        ...
        if (cacheFile.length() > 0) {
            ...
            editFile.readObject()?.run {
                (this as HashMap<String, HashMap<String, Object>>).forEach { pair ->
                    data[pair.key]!!.fromCacheData(pair.value)
                }
            }
            editFile.close()
        }
